...
## Autonomous adoption and Sidewinder refinement · 2026-09-12

Ethan accepted the sounds and Cobra/Vortex, preferred Tight grip, rejected Sidewinder motion quality, then authorized “implement all autonomoslu.” Tight grip now drives ordinary flight (1.5× steering, 2.4× velocity grip, 1.7× braking); sandbox overrides remain isolated and reset returns to Tight grip. The existing optional speed-coupled setting still layers its curve onto normal flight. The comparison button names the adopted baseline and shows its actual values even after selecting a different experiment.

Both Sidewinders replace opposing yaw/slide cuts with a 70° bank, one eased full roll during a smooth lateral displacement, recovery and boost-out. Cost remains 50%, duration 2.5 s. Smooth steps use an integrated pulse, so their displacement does not depend on tick boundaries. Constant-rate steps use the original integration; Cobra/Vortex content is byte-equivalent as parsed. [Original move table](../design/sidewinder-before-2026-09-12.json) preserves the previous choreography.

Verification: [build and five focused logs](flight-adoption-checks/) all pass. Actual Flight traces show each Sidewinder displaces 330 m laterally, mirrors its partner, returns to its entry heading and stays framed at wide/narrow aspects. Largest 60 Hz lateral velocity change is 11.88 m/s; largest rotation step is 9.0°. Checks cover paid keyboard paths, native campaign-vs-Tight-grip equivalence through turning/braking/roll, reset isolation, prior accepted move content, episode compatibility and unchanged audio lifecycle.

Native in-app browser: actual keys 3/4 triggered moves. Captured full sequences with screenshot wall-clock timings, reviewed [left sequence](../shots/2026-09-12-sidewinders/left-sequence.jpg) and [right sequence](../shots/2026-09-12-sidewinders/right-sequence.jpg), plus side/rear/chase and menu captures. The revised motion reads as one directional displacement with bank/roll/recovery (agent visual judgment, not owner acceptance). Capture playback used the supported free-flight mode because pointer lock is unavailable in the agent browser. Normal pointer-lock feel and the owner's revised-animation verdict remain unverified. The runtime preview is http://127.0.0.1:5209/?mission=proving-ground . No deployment.

Incidental inspection limitation: bare `?view=side|rear` can show empty sky when the default mission has a displaced spawn, because InspectView frames the origin. Explicit `mission=proving-ground` produces the correct inspection capture. Failed/blank captures are retained alongside successful captures; ship geometry was not changed by this pass.
