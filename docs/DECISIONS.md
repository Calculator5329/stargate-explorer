...
## 2026-09-12 — Layered audio bank and mix
REFINE: keep the persistent Audio owner and scene buses; replace individual placeholder voices and engine beds with an offline-authored sample bank. A louder synth-only revision risks retaining the same generic character. A general-purpose audio engine or local generative model would add machinery without establishing a better sound; a 27-clip CC0/authored bank is smaller and reviewable. Existing flight/travel/replay/UI meanings remain unchanged. Ethan's explicit allowance for generated/free-online audio supersedes the original no-audio-assets constraint for this pass. Kenney CC0 originals and license are retained with a deterministic numpy/ffmpeg generation script. Samples decode after a user gesture; missing assets fall back to existing synthesis. Independent saved mix controls, voice limits and a master compressor handle overlap. No deployment or external paid generation.

## 2026-09-12 — Adopt owner-selected flight and refine Sidewinders
Ethan authorized “implement all autonomoslu” after accepting sounds/Cobra/Vortex, preferring Tight grip and rejecting Sidewinder motion quality. Adopt Tight grip as normal handling, retain sandbox overrides and accepted sounds/moves. Pre-build interrogation: failure would be more unreadable spinning and accidental changes to accepted moves. Simpler alternative: remove opposing yaw/slide cuts, reuse the timeline for one lateral corkscrew with smooth thrust/rotation envelopes. Vocabulary stays Sidewinder/Tight grip. Fits existing content-driven moves and control scheme; no deployment. REFINE: optional smooth step envelopes only for Sidewinders, with legacy steps unchanged.
