...
- [x] Campaign data moves out of code: `src/content/campaign.json` holds the acts and every level (`mission/levels.ts` imports it, types it and checks it for duplicate ids, dangling requires and cycles); the dev server writes it back through `POST /__content/save` (`vite.config.ts`, paths fenced to `src/content/*.json`). *(2026-09-06, `scripts/chain-test.mjs` 17/17 on the JSON)*
- [x] Campaign board (`?edit=1`, `src/editor/board.ts`): a card per level on a draggable canvas, an arrow to the level it requires, act bands drawn round each act's cards (drop a card in another band to move it), threat pill per card, and the power curve under it in campaign order. *(2026-09-06, `scripts/editor-test.mjs`)*
- [x] Encounter composer (`src/editor/encounter.ts`, fields declared in `editor/schema.ts`): type with templates, system, act, chain (requires, unlocks), intro lines, every type's numbers, a wave editor with enemy-kind chips, a power readout with a "flown in" hull selector, duplicate, delete, new level, undo, save, FLY IT. *(2026-09-06)*
- [x] First draft of a three-act arc from the seventeen existing levels (FIRST FLIGHT, THE FLEET, MOTHERSHIPS) laid out on the board for Ethan to rearrange. *(2026-09-06; the chain itself is unchanged, only acts and positions are new)*
- [x] Prebuilt wave packs (`editor/packs.ts`, a shelf under the wave list and the wizard's opening-fight step) and a new-level wizard (`editor/wizard.ts`: kind, opening pack, system, follows, title). *(2026-09-06, Ethan's pick of the improvement list; `scripts/editor-test.mjs` covers both)*
- [x] Board viewport (pan, zoom, FIT), hover FLY tag on cards, story preview in the inspector. *(2026-09-06, the UI/UX refinement round)*
- [ ] Ethan arranges the draft: open `http://localhost:5187/?edit=1`, drag the arc into the shape he wants, fix the levels the curve flags, SAVE, and say what the designer is missing.
- [ ] Calibrate the power model: `editor/power.ts` has three constants (`HIT`, `ACC`, `EXPOSURE`) set by feel. Ethan (2026-09-06) does not want it tuned on his own flights alone; a headless fight with a scripted pilot, or several testers, before anyone trusts the decimals.
- [x] Move composer: a key-triggered sequence of manoeuvres (roll, hop, boost bursts, a snap turn) that spends boost energy; Ethan says this is more complicated than the first sketch, so it starts with a design packet (what a "move" is made of, what the energy is, how it reads on screen) before any editor. *(2026-09-06: packet answered, all seven calls; moves are step lists in `src/content/moves.json`, the boost bar pays, chord trigger with per-move keys, MOVES tab with step cards and TRY IT, sandbox level. `scripts/move-test.mjs`)*
- [ ] Ethan flies the PROVING GROUND (`?mission=proving-ground`, or PROVING GROUND ▸ on the MOVES tab) and says which numbers are wrong: the four starters, `moveAccel`/`moveDecel`, the 260 ms window, whether other families want their own keys.
- [ ] Enemies get moves (Ethan: player only first): the interpreter already ignores who owns it; an enemy needs a trigger policy in `Enemy.steer` and a bar of its own.
- [ ] Stat sheet: every ship and enemy kind on one editable page (`ships/registry.ts`, `core/tunables.ts` tables) with the derived numbers (dps, time to kill each kind, hull in glider rounds) next to the inputs.
- [ ] Ship upgrade and customisation (Ethan: "almost No Man's Sky level customisation"): the hangar becomes a place to swap parts and stats, not only hulls. Direction only; nothing starts before the stat sheet exists.
...
- [x] Gate travel no longer reloads the page: the sim, world, mission and rig are torn down and rebuilt in place at 0.6 s into the tunnel (`Travel.onSwap`, `main.ts` `build`/`teardown`, `render/dispose.ts`), the URL is rewritten with `history.replaceState`, audio and pointer lock survive; the reload path remains only for a page opened with `?arrive=1`. *(2026-09-06, `scripts/travel-test.mjs` and a stage-by-stage probe: HUD hidden through dial, tunnel and arrive, hub opens after the fade, no page errors)*
- [x] HUD text in the wormhole: `#hud.travel` hides every HUD layer (not only while the dial overlay is up) and the start screen shows once, not on every arrival. *(2026-09-06)*
- [x] Transition sound: engine ducked through the trip, a kawoosh burst and a wormhole sweep (looped noise lowpass sweep plus a sub sine) replace the missile-launch reuse, chevron blips softened; the arrival fade plays the tail of the sweep. *(2026-09-06; a synth guess until Ethan hears it)*
- [x] Prometheus hull pass 6 from Ethan's four reference images (`docs/refs/prometheus-references-2026-09-06.md`): one long flat box hull, open forward hangar boxes on the bow flanks, low raised deck with a stepped command tower and masts just aft of centre, greeble strips on every flank, a full-beam stern engine wall with four rectangular exhausts, one gunmetal grey. *(2026-09-06, `docs/shots/2026-09-06-hull-prometheus-*.png` against `2026-09-06-before-hull-prometheus-*.png`)*
- [x] Interceptors easier, gunboats harder: interceptor hp 20, slower cruise and dash, wider flinch, lower fire rate; gunboat hp 260, tighter fire cone, faster rounds cadence, damage 3.0; bomber hp 320. *(2026-09-06, `core/tunables.ts`; guesses until flown)*
- [x] Ha'tak shield nodes were unkillable: the three nodes sat inside the pyramid's 91 m collision sphere, so every round died on the "shielded" hull first. The capital now has a solid hit shape (pyramid, inverted underside, ring band; `Capital.clearance/contains/hitBy/pushOut`), `Lockable.hits` gives any target an exact hit test, and the player is pushed out along the nearest face instead of the sphere. *(2026-09-06, `scripts/hatak-probe.mjs`: node dead in 8 hits from 260 m, ship never inside the hull)*
- [x] Discreet settings: the start screen appears until the first FLY (`Settings.onboarded`), after that the game loads straight into the sortie and Esc toggles the menu. *(2026-09-06)*
- [x] Replays show every enemy that mattered (recorded by id in 16 slots, not by list index), rock breaks are replayed as bursts, both cameras park in clear space with a sight line to the kill and the ship (rock-aware), shot A leads the ship so the victim is in frame, shot B keeps the wreck framed after the kill. *(2026-09-06, `scripts/replay-test.mjs`: victim and kill on screen in shot A, B before and B after)*
- [ ] Ethan flies round 6: does the trip sound right, is the Prometheus now the ship in the pictures, are interceptors beatable and gunboats a threat, can he kill the three nodes, do the replays cover an asteroid kill.
...
- [x] Ethan picks a bet from the brainstorm. *(2026-09-06: breadth. Duel "super good idea", Hunt "really good idea", Intercept "okay, but less interesting, since they're free go ahead". Loadouts, combos, foot mode, boarding, surfaces and bases all go to Later below.)*
...
- [x] No native scrollbars anywhere in the game. *(2026-09-06, Ethan: "totally takes you out of it"; thin gold thumb on transparent track, `index.html`, CLAUDE.md hard convention; `docs/shots/2026-09-06-hub-scrollbar.png`)*
- [ ] Ancient-scroll treatment for the hub panel: parchment edges and a scroll-style thumb instead of the plain gold line (Ethan: "I love the ancient stargate scroll look"). Design lane, needs a mock in `docs/design/`.
- [ ] Ethan flies the duel (`?mission=duel&foe=<kind>`), the hunt and the bomber line; then the ace and bomber tables. The duel is the tuning bench for every kind.
...
- [ ] **Boarding** (brainstorm idea 7; Ethan: cool, hold off). Only after foot mode exists; the Ha'tak hangar as the first interior.
- [ ] **Planet surfaces** (Ethan: far out). Not before foot mode and not before the bases below have proven the gate-between-places loop.
