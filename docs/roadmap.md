...
- [ ] Move composer: a key-triggered sequence of manoeuvres (roll, hop, boost bursts, a snap turn) that spends boost energy; Ethan says this is more complicated than the first sketch, so it starts with a design packet (what a "move" is made of, what the energy is, how it reads on screen) before any editor.
