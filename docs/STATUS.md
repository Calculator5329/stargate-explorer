...
- **Kill replay** (`replay/replay.ts`): 10 s pose ring buffer plus the player's rounds, best-kill clip, two-shot slow-motion playback (drone pass, then over the shoulder toward the kill) with the rounds re-emitted into a visual-only tracer pool, from the end card (`V`) or the pause menu. Verified headless (`scripts/replay-test.mjs`, `docs/shots/2026-09-05-replay-shotB.png`).
...
- **Camera attitude lag** on pull-ups and barrel rolls (`T.camera.followRate`, `barrelFollow`). The 2026-09-05 flip through a barrel roll is fixed (up taken from the lagged frame, `barrelFollow` 18; `scripts/barrel-test.mjs`); Ethan has not flown the fix yet.
