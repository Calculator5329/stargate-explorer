...
for (let i = 0; i < 80 && kills === 0; i++) {
  ...
  kills = await page.evaluate(() => {
    ...
    // hold fire for the first 5 s of sim time so the clip gets its full pre-roll (the cut keeps PRE = 4.5 s before the kill)
    if (g.replay.t > 5) document.dispatchEvent(new MouseEvent("mousedown", { button: 0 }));
...
await until(0.75);
console.log("shotB", JSON.stringify(await probe()));
