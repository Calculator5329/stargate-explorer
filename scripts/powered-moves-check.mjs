...
import {MOVES,checkMoves} from '@/sim/moves';
...
const run=(move,aspect,agility=1)=>{
 ...
 if(move.id.startsWith('scissor')){assert(turn>Math.PI*3,'bank, cut, roll and recovery');assert(peakSide>85,'large vector-thrust displacement');assert(f.quat.angleTo(new Quaternion())<.05,'sidewinder recovers entry heading');}
 trails.dispose();
 return {id:move.id,aspect,agility,peakSpeed,peakSide,peakLift,turnDegrees:turn*180/Math.PI,maxScreen,frames,samples};
};
for(const move of MOVES){for(const aspect of [16/9,4/5])results.push(run(move,aspect));run(move,16/9,.7);}
console.log('PASS: paid multi-stage trajectories, real vector thrust, 180-degree reversal, 720-degree helix, mirrored cuts, all-hull rotation, landscape/portrait camera fit, bounded fading ribbons.');
