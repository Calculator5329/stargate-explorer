...
- **Movement goes through `Flight.velDir`/`pos`:** the nose and the velocity are separate; drift, bounce and the barrel hop rely on it. Player controls are fixed to classic throttle, cursor steering, raw mouse, flight assist on at 0.9, normal Y (Ethan, 2026-09-08; `FLIGHT_PROFILE` in `core/save.ts` overrides old saves). W/S throttle, A/D roll (double-tap = barrel roll), Shift boost, Space brake, Q/E strafe. Scheme/assist keyboard, gamepad and URL toggles are retired. Sensitivity and speed-coupled turning remain options. Paid moves may exceed normal thrust/rotation limits and spend boost energy; preserve the readable stunt camera and momentum on exit. Read keys through the scheme and `Input`'s action fields, never `Input.keys`; which physical key does what is `core/binds.ts` (the menu rebinds, the save carries it).
...
- **Resolution is decided in one place:** `Renderer.pixelRatio()` (tier dpr cap, tier pixel cap, dynamic scale). Never call `setPixelRatio` elsewhere. The overlay's `res N%` and `gpu X ms` are the tell when something is slow.
