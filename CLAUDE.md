...
Inspection views for asset work: `?view=side|top|front|rear` (static cam, ship rotates; `&spin=0` holds it). `?quality=low|med|high`, `?sky=abydos|deepSpace|chulak`.
...
src/
  ...
  world/   procedural skybox, planet, sun, debris, World composition
  ships/   parametric builder, ship defs, GLTF loader
...
## Working style
- Small, verifiable steps. After any visual change, capture a screenshot via `?view=side&spin=0` and `?view=rear&spin=0` into `docs/shots/YYYY-MM-DD-view.png` and compare against the previous one.
...
- Don't add dependencies without noting why in DECISIONS.md. Current runtime dep is just `three` (lil-gui comes from its examples).
